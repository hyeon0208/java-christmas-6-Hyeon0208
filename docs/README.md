...
### 메뉴
- [] 다음과 같은 메뉴가 있으며 메뉴 별로 Enum으로 관리 한다.
```agsl
<애피타이저>
...
- [] 12월 중 방문할 날짜 입력을 받는다.
  - [] 🚫 예외 : 숫자가 아닌 값을 입력했을 경우 `"[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요."` 메시지 출력
  - [] 🚫 예외 : 방문할 날짜가 1 이상 31 이하의 숫자가 아닐 경우 `"[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요."` 메시지 출력

- [] 주문할 메뉴와 개수를 입력 받는다.
  - [] `해산물파스타-2,레드와인-1` 같은 형식으로 주문할 메뉴와 개수를 하이픈 "-"로 구분하고, 다른 메뉴는 쉼표 ","로 구분한다.
    - [] 🚫 예외 : 메뉴 형식이 다를 경우 `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."` 메시지 출력
  - [] 🚫 예외 : 메뉴판에 없는 메뉴 입력 시 `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."` 메시지 출력
  - [] 🚫 예외 : 메뉴 개수에 숫자가 아닌 값을 입력 시 `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."` 메시지 출력
  - [] 🚫 예외 : 메뉴 개수에 1보다 작은 값을 입력 시 `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."` 메시지 출력
  - [] 🚫 예외 : 중복 메뉴를 입력 시 `ex: 시저샐러드-1,시저샐러드-3`  `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."` 메시지 출력
  - [] 🚫 예외 : 주문 개수가 20개를 넘어 갈 경우 `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."` 메시지 출력
  - [] 🚫 예외 : 음료만 주문할 경우 `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."` 메시지 출력
